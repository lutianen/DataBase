...
#define REDIS_NORMAL_JUDGE(reply)                                   \
    if (nullptr == reply) return Lute::ErrorCode::ERedis::NetError; \
    if (REDIS_REPLY_ERROR == reply->type) {                         \
        freeReplyObject(reply);                                     \
        return Lute::ErrorCode::ERedis::NormalError;                \
    }
...
bool RedisConn::connectSvr(const std::string &ip, uint16_t port,
