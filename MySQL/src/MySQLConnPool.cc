...
// 单例模式
// 利用局部静态变量的特性
Lute::mysql::MySQLConnPool* Lute::mysql::MySQLConnPool::getInstance() {
    static Lute::mysql::MySQLConnPool connPool;
    return &connPool;
}

MySQLConnPool::MySQLConnPool(int capicity, const std::string& ipAddr,
...
void MySQLConnPool::init(int capcity, const std::string& ipAddr, uint16_t port,
                         const std::string& user, const std::string& passwd,
                         const std::string& db, bool autoCommit,
                         const char* charset) {
    ipAddr_ = ipAddr;
    port_ = port;
    user_ = user;
...
/**
 * @brief 销毁当前连接池，即关闭连接池中所有的 MYSQL*
 *
 */
